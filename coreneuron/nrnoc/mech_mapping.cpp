...
extern "C" double* get_var_location_from_var_name(int mech_id,
                                                  const char* variable_name,
                                                  Memb_list* ml,
                                                  int node_index) {
    if (mechNamesMapping.find(mech_id) == mechNamesMapping.end()) {
        std::cerr << "ERROR [" << __FUNCTION__
                  << "] : no variable name mapping exist for mechanism id: " << mech_id
                  << std::endl;
        return NULL;
    }
    if (mechNamesMapping.at(mech_id).find(variable_name) == mechNamesMapping.at(mech_id).end()) {
        std::cerr << "ERROR [" << __FUNCTION__
                  << "] : no value associtated to variable name: " << variable_name << std::endl;
        return NULL;
    }
    int variable_rank = mechNamesMapping.at(mech_id).at(
        variable_name);  // at function can raise an out of range exeption
